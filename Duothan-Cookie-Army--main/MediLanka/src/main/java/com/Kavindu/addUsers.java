...
try {
		    ...
		    sql ="Insert into users(memberID,name,pharmacy,phone,email,add1,country,role) values('"+MemberID+"','"+name+"','"+dob+"','"+phone+"','"+email+"','"+address+"','"+country+"','"+role+"')";
		     int results =st.executeUpdate(sql);
		    if(results==0) {
		    	 request.setAttribute("status", "failedMember");
		    	 dispatcher = request.getRequestDispatcher("index.jsp");
				 
		     }else {
		    	   request.setAttribute("status", "successMember");
		    	   dispatcher = request.getRequestDispatcher("index.jsp");
		     }
